...
public class ChatWebsocketHandler extends TextWebSocketHandler {
	...
	// - WebSocketSession == 웹소켓에 연결된 클라이언트 세션이 들어있는것
	...
	// synchronizedSet메서드 : 동기화된 Set반환
	// -> 멀티스레드 환경에서 하나의 컬렉션 요소에 여러 스레드가 접근하면 충돌이 발생할 수 있으므로
	// 동기화(충돌이 안나도록 줄세움)를 진행
	
	// 싱글스레드 : 혼자서 일하는거, 멀티스레드 : 동시에 여러명이서 일하는거 (멀티태스킹같은거)
	
	
	
	// afterConnectionEstablished메서드 : 클라이언트와 연결이 완료되고, 통신할 준비가 되면 수행
	@Override
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {	
		
		// WebSocketSession메서 : 웹소켓에 접속/요청한 클라이언트의 세션
		System.out.println(session.getId() + "연결됨");
		
		sessions.add(session);
		//WebSocketSession을 Set에 추가
		// sessions(하늘색글씨)에는 채팅방에 접속한 사람만 담겨져 있을 예정.   나갔을 경우 sessions에서 삭제해야된다.
		// -> afterConnectionClosed 에서 처리
	}
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		// chat.js단에서 chattingSock.send 보낸거를 받는게 handleTextMessage다.
		
		// TextMessage : 웹소켓을 이용해 텍스트로 전달된 메세지가 담겨있는 객체
		
		// payload : 전송되는 데이터 
		
		// message.getPayload() : JSON형태 왜? 자바스크립트에서 그렇게 보냈으니까 
		
		System.out.println("전달된 메세지 : " + message.getPayload() );
		
		// Jackson 라이브러리 : Java에서 JSON을 다루기 위한 라이브러리
		
		// Jackson-databind 라이브러리 (우리가 쓸거) : 
		...
		ChatMessage chatMessage = objectMapper.readValue(message.getPayload(), ChatMessage.class);
														// message.getPayload() 읽어서  chatMessage vo 만들어줌
		
		
		
		// 시간 세팅  (vo에 세팅 안되있어서 해줘야함)
		...
		if ( result > 0 ) {
			...
			for(WebSocketSession s : sessions) {
				...
				int chatRoomNo = (Integer)s.getAttributes().get("chatRoomNo");
											//Controller에서 chatRoomNo @SessionAttributes 받고 있음
	...
	// afterConnectionClosed메서드 : 클라이언트와 연결이 종료되는 수행 
	@Override
